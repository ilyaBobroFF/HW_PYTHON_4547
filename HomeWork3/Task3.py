...
checkEnglish = "IFDGJLNRSYUVYQWXZ"
checkRussian = "БГДЁЖЗИЙЛПУФЦЧШЩЪЬЫЭЮЯ"
...
russian = False
for letter in word:
    for el in checkEnglish:
        if el == letter:
            english = True
            break
    for el in checkRussian:
        if el == letter:
            russian = True
            break
if english:
    for letter in word:
        sumscores += scoresEnglish[letter]
    print("Слово на английском языке")
    print(f"Количество очков = {sumscores}")
if russian:
    for letter in word:
        sumscores += scoresRussian[letter]
